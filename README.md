...
└── installer/
    ...
    ├── icons/
    │  	├── agentgateway.ico        # ARP / shortcut icon (Application)
...
Outputs:
```
installer\output\Machine\agentgateway-superuser-1.0.0.msi
installer\output\User\agentgateway-enduser-1.0.0.msi
```
...
Output: `installer\output\Machine\agentgateway-superuser-1.0.0.msi`

### 3. Silent install (no UI dialogs baked in)

Pass on the command line at install time without rebuilding:
```
msiexec /i agentgateway-enduser-1.0.0.msi /qn
```
